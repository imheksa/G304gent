          ...
          {isGuest && (
            <>
              {(["engines", "alerts", "facts"] as Tab[]).map((tab) => (
                <button
                  key={tab}
                  onClick={() => { window.location.href = "/G304gent/login"; }}
    ...
    <nav className="border-b border-white/5 bg-gray-950/80 backdrop-blur-xl">
      <div className="mx-auto flex h-14 items-center justify-between px-6 sm:px-10 lg:px-16 xl:px-24">
        ...
        <div className="flex items-center gap-4">
          ...
          <a href="/G304gent/login" className="rounded-lg bg-gradient-to-r from-cyan-500 to-violet-500 px-4 py-2 text-sm font-medium text-white hover:from-cyan-400 hover:to-violet-400 transition-all">
            Sign In
          </a>
        </div>
      </div>
